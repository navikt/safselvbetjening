...
public class DokumentoversiktSelvbetjeningVergemaalIT extends AbstractDokumentoversiktSelvbetjeningItest {

	@Test
	void shouldGetDokumentoversiktWhenTokenNotMatchingQueryIdentAndVergemaalExistsForTema() throws Exception {
		happyStubs("finnjournalposter_happy_gsak.json");
		stubReprApiRepresentasjon("repr-api-vergemaal-for.json");
	...
	@Test
	void shouldGetDokumentoversiktWhenTokenNotMatchingQueryIdentAndVergemaalExistsForTemaOnlyTemaInArguments() throws Exception {
		happyStubs();
		stubFagarkiv("finnjournalposter_happy_for_aap.json");
		stubSak("saker_happy_for_aap.json");
		stubReprApiRepresentasjon("repr-api-vergemaal-for-aap.json");
		...
		// vergemål FOR,AAP men kun tema FOR i filter
		assertThat(data.getTema().getFirst().getKode()).isEqualTo("FOR");
		assertThat(data.getTema().getFirst().getJournalposter()).hasSize(1);
	}

	@Test
	void shouldGetEmptyDokumentoversiktWhenTokenNotMatchingQueryIdentAndVergemaalDoesNotCoverTemaArgumentInQuery() throws Exception {
		happyStubs();

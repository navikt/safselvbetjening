...
class HentDokumentAuditTest {
	...
	private static final String REPRESENTANT = "22222222222";
	private static final String REPRESENTERT = DOKUMENTEIER;
	...
	void shouldMapHentDokumentAuditLogAsFullmektig() {
		...
		Fullmakt fullmakt = new Fullmakt(REPRESENTANT, REPRESENTERT, new LinkedHashSet<>(List.of("BAR", "FOR")));
	...
	@Test
	void shouldMapHentDokumentAuditLogAsVerge() {
		MDC.put(MDC_CONSUMER_ID, "itest:teamdokumenthandtering:audit");
		MDC.put(MDC_CALL_ID, "1234-5678-9101");

		Vergemaal vergemaal = new Vergemaal(REPRESENTANT, REPRESENTERT, new LinkedHashSet<>(List.of("BAR", "FOR")));
		HentdokumentRequest hentdokumentRequest = HentdokumentRequest.builder()
				.journalpostId(JOURNALPOST_ID)
				.dokumentInfoId(DOKUMENT_INFO_ID)
				.variantFormat(VARIANT_FORMAT)
		...
		CommonEventFormat commonEventFormat = audit.mapHentDokument(vergemaal, hentdokumentRequest);
		assertThat(commonEventFormat.toString())
				.isEqualTo("CEF:0|safselvbetjening|AuditLog|1.0|audit:access|brukers dokument hentet av verge|INFO|" +
						"end=1691755261001 sproc=itest:teamdokumenthandtering:audit devicePayloadId=1234-5678-9101 suid=22222222222 spriv=verge[BAR,FOR] " +
						"duid=11111111111 act=hentdokument_vergemaal flexString1=1000 flexString1Label=journalpostId flexString2=1001 flexString2Label=dokumentInfoId cs3=ARKIV cs3Label=variantFormat");
	}
